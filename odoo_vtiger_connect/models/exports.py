...
class Exports(models.Model):
    ...
    def _export_contact(self, records):
        ...
        if connection.get("status"):
            for contact in records:
                ...
                if res.get("status"):
                    ...
                    self._create_mapping("res.partner.mapping", mapData)
                    log_data.update(
                        remote_id = result.get("id"),
                        local_id = contact.id,
                        success = True,
                        error = "Successfully exported"
                    )
                else:
        ...
        if connection.get("status"):
            for lead in records:
                ...
                if res.get("status"):
                    ...
                    self._create_mapping("crm.lead.mapping", mapData)
                    log_data.update(
                        remote_id = result.get("id"),
                        local_id = lead.id,
                        success = True,
    ...
    def _export_product(self, records):
        api_url = f"{self.vt_base_url}/webservice.php"
        connection = self._vtiger_login()
        success, failure, log_line_data = [], [], []
        if connection.get("status"):
            for product in records:
                ...
                if res.get("status"):
                    ...
                    self._create_mapping("product.template.mapping", mapData)
                    log_data.update(
                        remote_id = result.get("id"),
                        local_id = product.id,
                        success = True,
    ...
    def _export_company(self, records):
        api_url = f"{self.vt_base_url}/webservice.php"
        connection = self._vtiger_login()
        success, failure, log_line_data = [], [], []
        if connection.get("status"):
